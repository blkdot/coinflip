import './App.css';
import { useEffect, useState } from 'react';
import { Contract, ethers } from 'ethers'

import CoinFlip from './artifacts/contracts/CoinFlip.sol/CoinFlip.json'
const CFAddress = `0x${process.env.REACT_APP_DEPLOYED_ADDRESS}`

// Move this to context?
const provider = new ethers.providers.Web3Provider(window.ethereum)
const contract = new ethers.Contract(CFAddress, CoinFlip.abi, provider)
const GASRATE = 0.8
const MINBETTINGBALANCE = 0.008
const WIN = 0
const LOSE = 1
const CANCELED = 2

function App() {
  

  const [walletconnected, setWalletconnected] = useState(false)
  const [player, setPlayer] = useState()
  const [myBalance, setMyBalance] = useState(0)
  const [contractBalance, setContractBalance] = useState(0)
  const [even, setEven] = useState("") 
  const [wager, setWager] = useState(0.01) 
  const [frontClass, setFrontClass] = useState("")
  const [backClass, setBackClass] = useState("")
  const [msg, setMsg] = useState("")
  const [msgClass, setMsgClass] = useState("")
  const [betClass, setBetClass] = useState("bet-btn")


  useEffect(() => {
    
  }, []);

  function events(){
    contract.on("GameResult", async (status, event) => {
      event.removeListener()
      switch ( status ){
        case WIN:
          setMsgClass("success")
          setMsg("You are Win!")
        break;
        case LOSE:
          setMsgClass("info")
          setMsg("You are Lose!")
        break;
        case CANCELED:
          setMsgClass("warn")
          setMsg("Sorry! Contract Balance is not enough")
        break;
      } 
    });
  }

  async function requestAccount() {
    try{
      debugger
      const accounts = await window.ethereum.request({ method: 'eth_requestAccounts' })
      setPlayer(accounts[0])
      setWalletconnected(true)
      let _myBalance = await provider.getBalance(accounts[0])
      setMyBalance(ethers.utils.formatEther(_myBalance))
      let _contractBalance = await contract.getDealerBalance()
      setContractBalance(ethers.utils.formatEther(_contractBalance))
    }catch (e){
      setWalletconnected(false)
      setMyBalance(0)
    }
  }
  function coinBtnClick(_isEven) {
    if (_isEven === "front"){
      setEven(true);
      setFrontClass("active")
      setBackClass("")
    }else{
      setEven(true);
      setFrontClass("")
      setBackClass("active")
    }
    setMsg("")
    setMsgClass("")
  }
  
  async function betCoinFlip() {
    if (typeof window.ethereum !== 'undefined') {
      //we need to connect wallet
      await requestAccount()
      //simple validation, eth for betting should be large than MINBETTINGBALANCE
      if (wager <= MINBETTINGBALANCE){
        setMsgClass("warn")
        setMsg(`Please enter the valid ETH for betting(Min Balance: ${MINBETTINGBALANCE})`)
        return
      }
      if (wager >= Math.min(myBalance, contractBalance) * GASRATE){
        setMsgClass("warn")
        setMsg(`Please enter the valid ETH for betting(Max Balance: ${MINBETTINGBALANCE})`)
        return
      }
      
      // the user should select the coin's front or back
      if (even === ""){
        setMsgClass("warn")
        setMsg("Please select coin")
        return
      }
      setMsgClass("")
      setMsg("")
      const provider = new ethers.providers.Web3Provider(window.ethereum)
      const signer = provider.getSigner()
      const betcontract = new ethers.Contract(CFAddress, CoinFlip.abi, signer)
      
      let updatedWager = ethers.utils.parseEther(wager.toString())
      setBetClass("bet-btn disabled")
      const tx = await betcontract.bet(even, { value: updatedWager })
      await tx.wait()
      setBetClass("bet-btn")
      events();
      let _myBalance = await provider.getBalance(player)
      setMyBalance(ethers.utils.formatEther(_myBalance))
      let _contractBalance = await contract.getDealerBalance()
      setContractBalance(ethers.utils.formatEther(_contractBalance))
    }
  }

  return (
    <div className="App">
      <header className="App-header">
        <h1>Ether Coin Flip</h1>
        <div className="">
          {walletconnected?(
            <div className="wallet-address">
              <img alt="metamask" className="metamask" src="data:image/svg+xml;base64,"/>
              {player}
            </div>
          ):(
            <button className="wallet-connect" onClick={requestAccount}>
              <img alt="metamask" className="metamask" src="data:image/svg+xml;base64,"/>
              Wallet Connect
            </button>
          )}
        </div>
        <h4>The best ethereum coin flip game of the world</h4>
        <div>
          <p>Contract Balance:</p>
          <p><strong>{contractBalance}</strong></p>
        </div>
        <div>
          <p>My Balance:</p>
          <p><strong>{myBalance}</strong></p>
        </div>
        <div>
          <p>Betting Range:</p>
          <p>{Math.min(MINBETTINGBALANCE, contractBalance)} ~ {Math.min(myBalance, contractBalance) * GASRATE}</p>
        </div>
        <label>ETH for Betting:</label><input className="wager-input" type="number" value={wager} onChange={e => setWager(e.target.value)} placeholder=""/>
        <div className="coin-btn-group">
          <button className={frontClass} onClick={e => coinBtnClick("front")}>
            <img alt="front" src="/img/coin-front.png"/>
          </button>
          <button className={backClass} onClick={e => coinBtnClick("back")}>
            <img alt="back" src="/img/coin-back.png"/>
          </button>
        </div>
        {walletconnected?(<button className={betClass} onClick={e => betCoinFlip()}>Bet</button>):("")}
        <p className={msgClass}>{msg}</p>
      </header>
    </div>
  );
}

export default App;